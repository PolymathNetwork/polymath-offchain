...
router.post('/email/confirm', async (ctx) => {
  ...
  const emailPin = await EmailPIN.findOneAndUpdate(
    { pin: String(pin).toLowerCase(), isConfirmed: false, createdAt: { $gte: minCreatedAt } }, { isConfirmed: true }
  )
  if (!emailPin) {
    ctx.body = {
      status: 'error',
  ...
  await User.findOneAndUpdate({ address: emailPin.address }, { email: emailPin.email })
...
router.post('/email', async (ctx) => {
  const { txHash, subject, body, coreVer, network, code, sig, address } = ctx.request.body
  ...
  const artifactFile = (name, ext = true) => '../.' + dir + name + (ext ? '.json' : '')
  try {
    const files = await readdirAsync(dir)
    files.forEach((file) => { // eslint-disable-next-line global-require, import/no-dynamic-require
  ...
  const initContract = (name, address) => { // eslint-disable-next-line global-require, import/no-dynamic-require
    const artifact = require(artifactFile(name))
    return new web3.eth.Contract(artifact.abi, address || artifact.networks[network].address)
  }
